AFRAME.registerComponent("atoms", {
  ...
  createAtoms: async function (element) {
    ...
    */
    ...
    for(var indice = 1; indice <= numOfElectron; indice++){
      //Crear órbita
      ...
      orbita.setAttribute("material", {color: "#ff9e80", opacity: 0.3})
     ...
     electron.setAttribute("id", `electron-group${elementName}`)
     ...
     orbita.appendChild(electron)

    //Crear la esfera del electron
    var electronEsfera = document.createElement("a-entity")
    electronEsfera.setAttribute("id",`electronEsfera-${elementName}`)
    electronEsfera.setAttribute("geometry", {
      primitive: "sphere",
      radius: 0.02
    })
    electronEsfera.setAttribute("material", {color: "#0d47a1", opacity: 0.6})
    electronEsfera.setAttribute("position",{
      x: 0.2, y:0.2, z:0
    })

    electron.appendChild(electronEsfera)
    } 
